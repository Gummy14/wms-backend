...
namespace WMS_API.Layers.Data
{
    public class ItemRepository : IItemRepository
    {
        ...
        {
            var result = await dBContext.ItemData.Where(x => x.NextEventId == null).ToListAsync();
            return result;
        }

        public async Task<ItemData> GetItemByIdAsync(Guid itemId)
        {
            var result = await dBContext.ItemData.FirstOrDefaultAsync(x => x.NextEventId == null && x.ItemId == itemId);
            return result;
        }
